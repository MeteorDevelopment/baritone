...
public class WaypointsCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        BiFunction<IWaypoint, Action, Component> toComponent = (waypoint, _action) -> {
            ...
            MutableComponent nameComponent = Component.literal(!name.isEmpty() ? name : tr("command.waypoints.empty"));
            ...
            component.setStyle(component.getStyle()
                    .withHoverEvent(new HoverEvent.ShowText(
                            Component.literal(tr("command.waypoints.clickSelect"))
                    ))
        ...
        if (action == Action.LIST) {
            ...
            if (waypoints.length > 0) {
                ...
                Paginator.paginate(
                        ...
                        () -> logDirect(
                                tag != null
                                        ? tr("command.waypoints.byTag", tag.name())
                                        : tr("command.waypoints.all")
                        ),
                        transform,
                        String.format(
                                "%s%s %s%s",
                ...
                throw new CommandInvalidStateException(
                        tag != null
                                ? tr("command.waypoints.noTag")
                                : tr("command.waypoints.none")
                );
            }
            ...
            MutableComponent component = Component.literal(tr("command.waypoints.added"));
            ...
            if (tag == null) {
                throw new CommandInvalidStateException(tr("command.waypoints.invalidTag", name));
            }
            ...
            MutableComponent textComponent = Component.literal(tr("command.waypoints.cleared", waypoints.length));
            ...
            logDirect(tr("command.waypoints.restored", waypoints.size()));
        } else {
            ...
            if (args.hasAny() && args.peekString().equals("@")) {
                ...
                switch (waypoints.length) {
                    case 0:
                        throw new CommandInvalidStateException(tr("command.waypoints.none"));
                    case 1:
            ...
            if (waypoint == null) {
                ...
                Paginator.paginate(
                        args,
                        waypoints,
                        () -> logDirect(tr("command.waypoints.multiple")),
                        transform,
                        String.format(
                                "%s%s %s %s",
                                FORCE_COMMAND_PREFIX,
                ...
                if (action == Action.INFO) {
                    ...
                    logDirect(tr("command.waypoints.position", waypoint.getLocation()));
                    MutableComponent deleteComponent = Component.literal(tr("command.waypoints.clickDelete"));
                    ...
                    MutableComponent goalComponent = Component.literal(tr("command.waypoints.clickGoal"));
                    ...
                    MutableComponent recreateComponent = Component.literal(tr("command.waypoints.clickRecreate"));
                    ...
                    MutableComponent backComponent = Component.literal(tr("command.waypoints.clickReturn"));
                    ...
                    MutableComponent textComponent = Component.literal(tr("command.waypoints.deleted"));
                    textComponent.setStyle(textComponent.getStyle().withClickEvent(new ClickEvent.RunCommand(
                            String.format(
                                    "%s%s restore @ %s",
                                    FORCE_COMMAND_PREFIX,
                    ...
                    logDirect(tr("command.waypoints.goal", goal));
                } else if (action == Action.GOTO) {
                    Goal goal = new GoalBlock(waypoint.getLocation());
                    baritone.getCustomGoalProcess().setGoalAndPath(goal);
                    logDirect(tr("command.waypoints.goingTo", goal));
                }
            }
        }
    }
    ...
    public String getShortDesc() {
        return tr("command.waypoints.shortDesc");
    }

    @Override
    public List<String> getLongDesc() {
        return Arrays.asList(tr("command.waypoints.longDesc").split("\n"));
    }
