...
/**
 * ponytail: 26.2 render port stub. 26.2 removed the immediate-mode draw path this used
 * (Tesselator, RenderType#draw(MeshData), com.mojang.blaze3d.platform.SourceFactor/DestFactor). Baritone's
 * path/selection/beacon-beam OVERLAY rendering is disabled here: geometry is still emitted into a throwaway
 * buffer so every caller keeps working unchanged (non-null buffers, same call flow), but nothing is drawn to
 * the screen. Core pathfinding/movement is unaffected - only the visual path preview is missing.
 *
 * Upgrade path: reimplement endLines/endBuffer against 26.2's PreparedRenderType draw model (see
 * net.minecraft.client.renderer.rendertype.RenderType#prepare) and a custom RenderPipeline for lines.
 */
public interface IRenderer {
    ...
    // ponytail: overlay rendering disabled on 26.2 - build and discard, no GPU draw.
    ...
    // ponytail: beacon-beam overlay disabled on 26.2; only ever passed to the no-op endBuffer above.
